...
?>
<div class="card">
  <div class="card-header"><h5 class="card-header-text mb-0">Reminders</h5></div>
  <div class="card-block" style="padding:1rem;">
    ...
    <?php else: ?>
      <div class="mb-3" style="font-size:13px;color:#555;">
        ...
        <strong>Land:</strong> <?= htmlspecialchars($land['land_address'] ?? ('Land #' . (int)($land['land_id'] ?? 0))) ?>
      </div>

      <div class="reminder-item" style="border:1px solid #ddd;padding:12px 14px;border-radius:6px;margin-bottom:14px;background:#fafafa;">
        <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;">
          ...
          <div style="margin-top:8px;">
            <label for="ltl-recovery-date" style="font-size:12px;font-weight:600;margin-right:6px;">As At Date</label>
            <input type="date" id="ltl-recovery-date" class="form-control form-control-sm d-inline-block" style="width:160px;" value="<?= date('Y-m-d') ?>" />
            <button type="button" class="btn btn-outline-primary btn-sm" id="ltl-recovery-letter-btn" style="margin-left:6px;">
...
function printAnnexure12A(lang) {
    ...
    const url = `letters/ltl_annexure_12A.php?id=<?= urlencode($md5) ?>&as_at_date=${date}&last_reminder_date=${lastReminderDate}&language=${lang}`;
    window.open(url, '_blank');
}
</script>
      ...
      <div class="reminder-item" style="border:1px solid #ddd;padding:12px 14px;border-radius:6px;background:#fff;">
        <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;margin-bottom:10px;">
          <div>
            <span style="font-weight:600;font-size:14px;">Payment Reminders</span><br>
            <small style="color:#666;">Track sent reminders. Add new entries and cancel if needed.</small>
          </div>
          <div style="margin-top:8px;display:flex;align-items:center;gap:6px;flex-wrap:wrap;">
            <select id="rem-type" class="form-control form-control-sm" style="width:160px;">
            ...
            <input type="date" id="rem-date" class="form-control form-control-sm" style="width:150px;" value="<?= date('Y-m-d') ?>" />
            <button type="button" id="rem-add-btn" class="btn btn-success btn-sm" disabled title="Select type & date"><i class="fa fa-plus"></i> Add</button>
          </div>
        </div>
        <style>
          .rem-table th, .rem-table td { font-size:13px; }
          .rem-row-cancelled { background:#fde2e2 !important; }
          .rem-row-cancelled td { color:#842029 !important; }
        </style>
      ...
      <script>
        (function(){
          ...
          if (rDate) { rDate.addEventListener('change', function(){ /* placeholder if future logic needed */ }); }
          if (rBtn) {
            rBtn.addEventListener('click', function(){
              if (!rDate.value){ Swal.fire('Validation','Select date first','warning'); return; }
              var url = 'letters/lease_recovery_letter.php?id=<?= urlencode($md5) ?>&date=' + encodeURIComponent(rDate.value) + '&_ts=' + Date.now();
              window.open(url,'_blank');
            });
          }
          ...
          function validateRemInputs(){
            if (remTypeEl && remDateEl && remTypeEl.value && remDateEl.value){
              remAddBtn.disabled = false; remAddBtn.title='Add';
            } else {
              remAddBtn.disabled = true; remAddBtn.title='Select type & date';
            }
          }
          ...
          function loadReminders(){
            if (!LEASE_ID){ remBody.innerHTML='<tr><td colspan="5" class="text-danger text-center">No lease.</td></tr>'; return; }
            ...
            fetch('ltl_ajax/list_reminders.php?lease_id=' + LEASE_ID + '&_ts=' + Date.now())
              .then(r=>r.text())
              .then(html=>{ remBody.innerHTML = html; bindCancelReminders(); validateRemInputs(); })
              .catch(()=>{ remBody.innerHTML='<tr><td colspan="5" class="text-danger text-center">Load failed.</td></tr>'; });
          }
            ...
            remBody.querySelectorAll('.rem-cancel-btn').forEach(function(btn){
              btn.addEventListener('click', function(){
                ...
                var doCancel = function(){
                  ...
                  fetch('ltl_ajax/cancel_reminder.php', {method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'}, body:fd.toString()})
                    .then(r=>r.json())
                    .then(resp=>{ if(resp.success){ loadReminders(); } else { Swal.fire('Error', resp.message||'Cancel failed', 'error'); } })
                    .catch(()=> Swal.fire('Error','Network error','error'));
                };
                if (window.Swal){
                  Swal.fire({title:'Cancel this reminder?', icon:'warning', showCancelButton:true}).then(function(res){ if(res.isConfirmed) doCancel(); });
                } else {
          ...
          if (remAddBtn){
            remAddBtn.addEventListener('click', function(){
              ...
              var t = remTypeEl.value; var d = remDateEl.value;
              if (!t || !d){ Swal.fire('Validation','Select type and date','warning'); return; }
              remAddBtn.disabled = true; remAddBtn.innerHTML='<i class="fa fa-circle-o-notch fa-spin"></i> Saving...';
              var fd = new URLSearchParams(); fd.append('lease_id', LEASE_ID); fd.append('reminders_type', t); fd.append('sent_date', d);
              fetch('ltl_ajax/add_reminder.php', {method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'}, body:fd.toString()})
                .then(r=>r.json())
                .then(resp=>{
                  if (resp.success){
                    if (window.Swal){ Swal.fire({icon:'success', title:'Added', timer:1200, showConfirmButton:false}); }
                    // reset type only (date stays for convenience)
                    remTypeEl.value=''; validateRemInputs(); loadReminders();
                  } else { Swal.fire('Error', resp.message||'Insert failed', 'error'); }
                })
                .catch(()=> Swal.fire('Error','Network error','error'))
                .finally(()=>{ remAddBtn.disabled=false; remAddBtn.innerHTML='<i class="fa fa-plus"></i> Add'; });
            });
          }
