...
<form id="ltl_land_form" class="mb-3">
  ...
  <div class="row">
    ...
    <div class="col-md-3">
      <div class="form-group">
        ...
        <input type="text" id="ltl_land_address" name="land_address" class="form-control" placeholder="Enter address" required>
      </div>
    </div>
  ...
  <div class="row">
    ...
    <div class="col-md-2">
      <div class="form-group">
        ...
        <input type="number" step="any" id="ltl_extent" name="extent" class="form-control" placeholder="Enter extent">
      </div>
    </div>
    <!-- Extent Unit -->
    <div class="col-md-2">
  ...
  <div class="row">
    <div class="col-md-6">
      ...
      <button type="button" id="ltl_add_line" class="btn btn-outline-secondary btn-sm mt-2"><i class="fa fa-plus"></i> Add Line</button>
    </div>
    <div class="col-md-6">
      <div id="ltl_map" style="height:300px; border:1px solid #ddd;"></div>
      <small class="text-muted">Click on the map to append a coordinate pair. Use the list to edit values. Polygon preview updates automatically.</small>
    </div>
  </div>

  <div class="text-right mt-3">
<?php if (hasPermission(21)): ?>
    <button type="button" class="btn btn-outline-primary mr-2" id="ltl_edit_btn"><i class="fa fa-edit"></i> Edit</button>
    <?php endif; ?>
    <button type="submit" class="btn btn-success" id="ltl_save_btn"><i class="fa fa-save"></i> Save Land Information</button>
  </div>
</form>
...
<script>
  (function(){
    ...
    // Select2 for DS/GN
    if (window.jQuery) {
      $('#ltl_ds_id').select2({ width: '100%' });
      $('#ltl_gn_id').select2({ width: '100%' });

      $('#ltl_ds_id').on('change', function(){
        var ds = $(this).val();
        if (ds) {
          $.get('ajax/get_gn_divisions.php', { c_id: ds }, function(html){
    ...
    function ensureMinPoints(n){
      while(boundaryPoints.length < n){ boundaryPoints.push({lat:'', lng:''}); }
    }
    function renderBoundaryList(){
      ...
      boundaryPoints.forEach(function(p, idx){
        html += '<tr>' +
                '<td><input type="text" class="form-control form-control-sm ltl-lat" placeholder="Latitude" data-index="'+idx+'" value="'+(p.lat||'')+'"></td>' +
                '<td><input type="text" class="form-control form-control-sm ltl-lng" placeholder="Longitude" data-index="'+idx+'" value="'+(p.lng||'')+'"></td>' +
                '</tr>';
      });
      ...
      document.querySelectorAll('#ltl_boundary_list .ltl-lat').forEach(function(inp){
        inp.addEventListener('input', function(){
          ...
          if (!boundaryPoints[i]) boundaryPoints[i] = {lat:'', lng:''};
    ...
    function initMap(){
      ...
      try {
        var dsGeo = { type: 'Feature', properties: { NAME_2: CLIENT_NAME, TYPE_2: 'Division' }, geometry: { type: 'MultiPolygon', coordinates: COORDS } };
        dsLayer = L.geoJSON(dsGeo, { style: { color: '#d32f2f', weight: 2, fillColor: '#ef5350', fillOpacity: 0.05 } }).addTo(map);
        if (dsLayer && dsLayer.getBounds && dsLayer.getBounds().isValid()) {
          map.fitBounds(dsLayer.getBounds());
        }
      } catch(e) { /* ignore if malformed */ }
      map.on('click', function(e){
        ...
        for (var i = 0; i < limit; i++) {
          if (!boundaryPoints[i] || !boundaryPoints[i].lat || !boundaryPoints[i].lng) { indexToFill = i; break; }
        }
        if (indexToFill === -1) {
          // All existing rows complete: append a new one
          boundaryPoints.push({ lat: lat, lng: lng });
        } else {
          if (!boundaryPoints[indexToFill]) boundaryPoints[indexToFill] = {lat:'', lng:''};
    ...
    function drawPolygon(){
      ...
      var pts = boundaryPoints
        .map(function(p){ var lat = parseFloat(p.lat), lng = parseFloat(p.lng); return (isFinite(lat) && isFinite(lng)) ? [lat, lng] : null; })
        .filter(Boolean);
      if (polygonLayer) { polygonLayer.remove(); polygonLayer = null; }
      if (pts.length >= 3) {
        polygonLayer = L.polygon(pts, { color: '#1976d2', weight: 2, fillColor: '#64b5f6', fillOpacity: 0.25 }).addTo(map);
        try { map.fitBounds(polygonLayer.getBounds(), { padding: [10,10] }); } catch(e){}
      } else if (pts.length >= 2) {
        polygonLayer = L.polyline(pts, { color: '#1976d2', weight: 2 }).addTo(map);
      }
    }
    ...
    function serializeBoundary(){
      var pts = boundaryPoints
        .map(function(p){ var lat = parseFloat(p.lat), lng = parseFloat(p.lng); return (isFinite(lat) && isFinite(lng)) ? {lat: lat, lng: lng} : null; })
        .filter(Boolean);
      if (pts.length === 0) {
        // store 4 editable placeholders when nothing valid entered yet
        var placeholders = Array.from({length:4}).map(function(){ return {lat: '', lng: ''}; });
    ...
    function getQueryParam(k){
      var params = new URLSearchParams(window.location.search); return params.get(k);
    }
    function preloadIfAny(){
      ...
      fetch(url)
        ...
        .then(function(resp){
          ...
          if (d.ben_id) { $('#ltl_ben_id').val(d.ben_id); }
          $('#ltl_ds_id').val(d.ds_id || '').trigger('change');
          // Load GN list then set
          $.get('ajax/get_gn_divisions.php', { c_id: d.ds_id }, function(html){
            $('#ltl_gn_id').html(html);
          ...
          try {
            ...
            if (Array.isArray(arr)) {
              arr.forEach(function(pt){ boundaryPoints.push({ lat: pt.lat, lng: pt.lng }); });
            }
          } catch(e) {}
          ensureMinPoints(4);
          renderBoundaryList();
          // If this is an existing record, default to read-only mode
          if (d.land_id) { setEditable(false); }
          $('#ltl_boundary_list input').prop('disabled', !EDITABLE);
      ...
      $.post('ltl_ajax/save_land.php', data)
        .done(function(resp){
          if (resp && resp.success) {
            if (resp.land_id) { $('#ltl_land_id').val(resp.land_id); }
            Swal.fire({ icon: 'success', title: 'Saved', text: resp.message || 'Land information saved', timer: 1800, showConfirmButton: false });
          } else {
        ...
        .always(function(){ $('#ltl_save_btn').prop('disabled', false); });
    });
...
<style>
  ...
  .leaflet-grab, .leaflet-dragging .leaflet-grab { cursor: default !important; }
  .leaflet-marker-icon { cursor: default !important; }
  /* Optional: increase click target tolerance slightly */
  #ltl_map { touch-action: manipulation; }
  /* Keep the map visible with a subtle border */
  #ltl_map { border: 1px solid #ddd; }
  /* Two-column boundary table tidy spacing */
  #ltl_boundary_list table td { padding: .25rem .5rem; }
  #ltl_boundary_list input { height: 30px; }
  #ltl_boundary_list thead th { font-weight: 600; }
